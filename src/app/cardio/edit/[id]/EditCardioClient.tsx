...
export default function EditCardioLogPage() {
  ...
  // Load existing cardio log
  useEffect(() => {
    const loadCardioLog = async () => {
      setIsLoading(true);
      const { data, error: loadError } = await getCardioLogById(logId);

      if (loadError || !data) {
        setError(loadError?.message || 'Failed to load cardio log');
        setIsLoading(false);
        return;
      }
      ...
      setAverageHeartRate(data.average_heart_rate ?? '');
      setMaxHeartRate(data.max_heart_rate ?? '');
      setIntervals(data.intervals);
      setIntervalDescription(data.interval_description ?? '');
      setNotes(data.notes ?? '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const input: UpdateCardioLogInput = {
      ...
      const { data, error: submitError } = await updateCardioLog(logId, input);
    ...
    return (
      <div className="min-h-screen bg-background p-4 md:p-6">
        <div className="max-w-4xl mx-auto">
          <Card className="p-8">
            <p className="text-white/60 text-center">Loading cardio log...</p>
          </Card>
        </div>
      </div>
    );
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="p-8 border-red-500/20 bg-red-500/5">
            ...
            <div className="flex justify-center">
              <Button onClick={() => router.push('/cardio')}>
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-6">
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-white mb-2">Edit Cardio Session</h1>
          <p className="text-white/60">Update your conditioning session details</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex gap-3">
            <Button type="submit" disabled={isSubmitting} className="flex-1">
              ...
              {isSubmitting ? 'Saving...' : 'Update Cardio Log'}
            </Button>
            <Button
              type="button"
              variant="ghost"
