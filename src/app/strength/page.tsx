...
export default function StrengthPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      // Load user's exercise names for filter
      const exercisesData = await getUserExercises(user.id);
      setUserExercises(exercisesData);

      // Set default exercise for progress chart
      if (exercisesData.length > 0 && !selectedProgressExercise) {
        setSelectedProgressExercise(exercisesData[0]);
      }
    } catch (error) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-white">Strength Training</h1>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
          ...
          <Card className="p-4">
            ...
            <div className="text-2xl font-bold text-blue-400">{stats.workoutsThisWeek}</div>
          </Card>
          <Card className="p-4">
          ...
          <Card className="p-4">
            <div className="text-gray-400 text-xs mb-1">Week Volume</div>
          ...
          <Card className="p-4">
            ...
            <div className="text-2xl font-bold text-white">{stats.activeExercises}</div>
          </Card>
          <Card className="p-4">
            <div className="text-gray-400 text-xs mb-1">PRs (30d)</div>
            <div className="text-2xl font-bold text-yellow-400">{recentPRs.length}</div>
          </Card>
        </div>

        {/* Recent PRs */}
        {recentPRs.length > 0 && (
          <Card className="p-6 mb-6 bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border-yellow-500/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {recentPRs.slice(0, 6).map(pr => (
                <div key={pr.id} className="flex items-center justify-between p-3 bg-[#0f0f13] rounded-lg">
                  <div>
                  ...
                  <div className="text-right">
                    <div className="text-yellow-400 font-bold">{pr.value.toFixed(1)} lbs</div>
                    {pr.previous_value && (
