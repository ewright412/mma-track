  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        discipline: input.is_rest_day ? null : input.discipline || null,
...
export async function matchSessionToSchedule(
  ...
  try {
    ...
    // Find matching entries (same day, same discipline, not rest day)
    const dayEntries = template.entries.filter(
      e => e.day_of_week === dayOfWeek && e.discipline === discipline && !e.is_rest_day
    );

    if (dayEntries.length === 0) return { matched: false, error: null };

    // Check which entries don't have adherence yet for this date
    ...
    // Calculate expected duration from start/end times
    const [startH, startM] = unmatchedEntry.start_time.split(':').map(Number);
    const [endH, endM] = unmatchedEntry.end_time.split(':').map(Number);
    const expectedMinutes = (endH * 60 + endM) - (startH * 60 + startM);

    // Determine status: >= 80% of expected = completed, otherwise partial
  ...
  try {
    ...
    const entriesWithAdherence: ScheduleEntryWithAdherence[] = dayEntries.map(entry => {
      ...
      if (record) {
        ...
        // Future entries aren't missed yet - only past times are missed
        // For simplicity on dashboard, show as 'missed' only if we're past the scheduled time
        const now = new Date();
        ...
        if (now < entryEndTime) {
          // Haven't reached end time yet - treat as upcoming (not missed)
          adherenceStatus = 'missed'; // We'll handle the display in the component
        }
      }
